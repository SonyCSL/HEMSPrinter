...
全体構成がOverview.pngにあります。

○使用したハードウェア・ソフトウェア

ハードウェア
・レシートプリンタ　スター精密 SM-S210i Amazonにて34,900円 (2015/10/13現在)
...
・焦電型赤外線（人感）センサーモジュール　ＳＢ４１２Ａ（秋月電子で500円http://akizukidenshi.com/catalog/g/gM-09002/）
・Windows PC (エミュレータとProcessingの実行に用いました）
・LED照明つき押しボタン（秋葉原ガード下で1100円くらいで購入。LEDは3vで十分光るものに入れ替えています。

ソフトウェア
・mosquitto (MQTT Broker. Port12345を使用)
・node.js (MQTT-Socket.ioの変換用.Port12346と12347を使用。express,mqtt,socket.ioの３つのパッケージを入れています。）
・nginx (Web server。以上３つはAWS上に構築しました。kadecotgallery.xyz:80を使用

・StarWebPRNT Browser 有料版。(Google Playにて3600円。Android端末に入れて使います）
https://play.google.com/store/apps/details?id=com.starmicronics.starwebprntpaid
・Kadecot　（Google Playから無料で落とせます。
https://play.google.com/store/apps/details?id=com.sonycsl.Kadecot


○使い方（hemsgallery.xyzと書いてある所は、app/webがインストールされたサーバー名に適宜読み替えてください。
・hemsgallery.xyz上でnginxを立ち上げます。apacheとかでも問題ありません。
・hemsgallery.xyz上でmosquittoを立ち上げます。
mosquitto -v -p 12345
・hemsgallery.xyz上でnode.jsを立ち上げます。
node mq/runnode.js
・宅内のPC上でダイワハウスのエミュレータを立ち上げます。
・同一PC上でProcessingのHEMSPrinter2を立ち上げます。（ソースコード内のIPアドレスを、エミュレータの、すなわち自分のIPアドレスに書き換えてください）
...
・http://kadecotgallery.xyz/KadecotBTClient.htmlを開きます。URL末尾に?kip=[kadecot ip]を追加してください([Kadecot ip]は実際のIPアドレスに変更してください)。最初にOAuth認証があります。認証が終わったら、デバッグコンソールに機器一覧が出ていることを確認します。
...
○開発したもの
・Webアプリ：web/フォルダ以下に置いてあります。現在は動く状態でhttp://kadecotgallery.xyz以下にインストールされていますが、将来変更の可能性があります。
...
・Kadecot Lightblue Bean Plugin : lightbluebeanplugin
...
・Processing：Processing/HEMSPrinter2/HEMSPrinter2.pde
...
　プログラム：LightblueBean/HEMSPrinter2/HEMSPrinter2.inoを、Windows用Bean Loaderで端末に流し込みます。ボードの名前を「HEMSPrinter」にしておく必要があります。
...
　プログラム：LightblueBean/DoorSensor/DoorSensor.inoにあります。人感センサーだけでなく、ボード上に最初から載っている加速度センサーを用いています。玄関ドアに取り付けることを想定しています。人感センサーが反応する前に加速度センサーが反応した場合は、外から人が入ってきたと考えられるので、帰宅信号を出します。逆に、人感センサーが先に反応したら外出しようとしていると考えられるので、外出信号を出します。（後者は、玄関掃除をしているときなどに誤認識の可能性があります）